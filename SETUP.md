...
В репозитории есть **2 пресета** окружения: `dev.compose.env` и `prod.compose.env`.
Файл `compose.env` **не коммитится** (в `.gitignore`) и используется `docker-compose.yml`.

### Запуск dev / prod (явный выбор окружения)
...
Скрипт:
- копирует выбранный пресет в `compose.env`
- экспортирует переменные, чтобы **build-аргументы фронта** (например `NEXT_PUBLIC_API_URL`) попали в сборку
- запускает `docker compose up -d --build`

### Полный сброс БД (с чистого листа)

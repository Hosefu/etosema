...
import { Layout, Card, Form, InputNumber, Select, ColorPicker, Button, Upload, List, Tabs, message, Row, Col, Typography, Divider, Slider, Radio } from 'antd';
import { UploadOutlined, DeleteOutlined, SaveOutlined } from '@ant-design/icons';
import { adminGetDesign, adminUpdateDesign, adminUploadFont, adminDeleteFont, DesignSettings, Font, TypographyConfig } from '@/lib/adminClient';
...
const defaultSettings: DesignSettings = {
  id: 0,
  typography: {
    body: { ...defaultTypography, size: 18, lineHeight: 135, letterSpacing: -3 },
    headingSmall: { ...defaultTypography, size: 26, lineHeight: 100, letterSpacing: -3 },
    headingLarge: { ...defaultTypography, size: 90, lineHeight: 90, letterSpacing: -3 },
  },
  colors: {
    background: '#f5f5f5',
    card: '#ffffff',
...
const TypographyEditor = ({ value, onChange, fonts, label }: TypographyEditorProps) => {
  const handleChange = (key: keyof TypographyConfig, val: any) => {
    onChange({ ...value, [key]: val });
  };

  const fontOptions = [
    { label: 'System Fonts', options: [
    ...
    { label: 'Uploaded Fonts', options: fonts.map(f => ({ label: f.name, value: f.family })) }
  ];

  return (
    <div style={{ marginBottom: 24, padding: 16, border: '1px solid #f0f0f0', borderRadius: 8 }}>
      <Text strong style={{ display: 'block', marginBottom: 16 }}>{label}</Text>
      <Row gutter={[16, 16]}>
        <Col span={12}>
...
export default function DesignPage() {
    ...
    try {
      ...
      if (res.success && res.data) {
        setSettings({
          ...
          typography: {
            body: { ...defaultSettings.typography.body, ...res.data.settings.typography.body },
            headingSmall: { ...defaultSettings.typography.headingSmall, ...res.data.settings.typography.headingSmall },
            headingLarge: { ...defaultSettings.typography.headingLarge, ...res.data.settings.typography.headingLarge },
          },
  ...
  const handleFontUpload = async (options: any) => {
    ...
    try {
      const res = await adminUploadFont(file, { family: file.name.split('.')[0] });
      if (res.success) {
      ...
      <div style={{ display: 'flex', gap: 24, alignItems: 'flex-start' }}>
        ...
        <div style={{ flex: 1, maxWidth: 600 }}>
          <Card 
            ...
            extra={<Button type="primary" icon={<SaveOutlined />} loading={saving} onClick={handleSave}>Сохранить</Button>}
          >
                ...
                children: (
                  <>
                    <TypographyEditor 
                      ...
                      onChange={v => setSettings({ ...settings, typography: { ...settings.typography, body: v } })}
                      fonts={fonts}
                    />
                    <TypographyEditor 
                      label="Небольшой заголовок" 
                      value={settings.typography.headingSmall} 
                      onChange={v => setSettings({ ...settings, typography: { ...settings.typography, headingSmall: v } })}
                      fonts={fonts}
                    />
                    <TypographyEditor 
                      label="Большой заголовок" 
                      value={settings.typography.headingLarge} 
                      onChange={v => setSettings({ ...settings, typography: { ...settings.typography, headingLarge: v } })}
                      fonts={fonts}
                    />
                  </>
                )
                  ...
                  <div style={{ padding: 16 }}>
                    <Form.Item label="Фон страницы">
                      <ColorPicker 
                        ...
                        onChange={(c, hex) => setSettings({ ...settings, colors: { ...settings.colors, background: hex } })} 
                        showText 
                      />
                    </Form.Item>
                    <Form.Item label="Фон карточек">
                      <ColorPicker 
                        ...
                        onChange={(c, hex) => setSettings({ ...settings, colors: { ...settings.colors, card: hex } })} 
                        showText 
                      />
                    </Form.Item>
                  </div>
                      ...
                      <Col span={12}>
                        <Form.Item label="Скругление (px)">
                          <InputNumber 
                            ...
                            onChange={v => setSettings({ ...settings, cards: { ...settings.cards, borderRadius: v || 0 } })} 
                            style={{ width: '100%' }}
                          />
                        </Form.Item>
                      </Col>
                      <Col span={12}>
                        <Form.Item label="Фикс. высота (0 = авто)">
                          <InputNumber 
                            ...
                            onChange={v => setSettings({ ...settings, cards: { ...settings.cards, height: v || 0 } })} 
                            style={{ width: '100%' }}
                          />
                        </Form.Item>
                      </Col>
                      ...
                      <Col span={24}>
                        <Form.Item label="Внутренний отступ (px)">
                          <InputNumber 
                            ...
                            onChange={v => setSettings({ ...settings, cards: { ...settings.cards, padding: v || 0 } })} 
                            style={{ width: '100%' }}
                          />
                        </Form.Item>
                      </Col>
              ...
              },
              {
                ...
                children: (
                  <div style={{ padding: 16 }}>
                    ...
                    <Row gutter={16} style={{ marginTop: 8, marginBottom: 24 }}>
                      <Col span={12}>
                        <Form.Item label="Внешний отступ (Margin)">
                          <InputNumber 
                            value={settings.grid.margin} 
                            onChange={v => setSettings({ ...settings, grid: { ...settings.grid, margin: v || 0 } })} 
                            style={{ width: '100%' }}
                          />
                        </Form.Item>
                      </Col>
                      <Col span={12}>
                        <Form.Item label="Межколонник (Gutter)">
                          <InputNumber 
                            ...
                            onChange={v => setSettings({ ...settings, grid: { ...settings.grid, gutter: v || 0 } })} 
                            style={{ width: '100%' }}
                          />
                        </Form.Item>
                      </Col>
                    ...
                    <Form.Item label="Ширина текста (колонок)" style={{ marginTop: 8, marginBottom: 24 }}>
                      <Radio.Group 
                        value={settings.grid.textColumns || 8} 
                        onChange={e => setSettings({ ...settings, grid: { ...settings.grid, textColumns: e.target.value } })}
                        buttonStyle="solid"
                      >
                    ...
                    <Form.Item label="Флаг текста (Text Align)">
                      <Radio.Group 
                        ...
                        onChange={e => setSettings({ ...settings, grid: { ...settings.grid, textAlign: e.target.value } })}
                        buttonStyle="solid"
                      >
                    ...
                    <Form.Item label="Положение блока (Block Position)">
                      <Radio.Group 
                        ...
                        onChange={e => setSettings({ ...settings, grid: { ...settings.grid, blockAlign: e.target.value } })}
                        buttonStyle="solid"
                      >
                        <Radio.Button value="left">Left</Radio.Button>
                        <Radio.Button value="center">Center</Radio.Button>
                    ...
                    <Form.Item label="Базовый отступ (Gap, px)" style={{ marginTop: 8 }}>
                      <InputNumber 
                        value={settings.spacing.baseGap} 
                        onChange={v => setSettings({ ...settings, spacing: { ...settings.spacing, baseGap: v || 0 } })} 
                        style={{ width: '100%' }}
                      />
                    </Form.Item>
                  </div>
                  ...
                  <div style={{ padding: 16 }}>
                      ...
                      <Col span={12}>
                        <Form.Item label="Отступ линии (px)">
                          <InputNumber 
                            ...
                            onChange={v => setSettings({ ...settings, links: { ...settings.links, offset: v || 0 } })} 
                            style={{ width: '100%' }}
                          />
                        </Form.Item>
                      </Col>
                      <Col span={12}>
                        <Form.Item label="Толщина линии (px)">
                          <InputNumber 
                            ...
                            onChange={v => setSettings({ ...settings, links: { ...settings.links, thickness: v || 1 } })} 
                            min={1}
                            style={{ width: '100%' }}
                          />
                        </Form.Item>
                    ...
                    <Form.Item label="Цвет подчеркивания">
                      <ColorPicker 
                        ...
                        onChange={(c, hex) => setSettings({ ...settings, links: { ...settings.links, color: hex } })} 
                        showText 
                      />
                    </Form.Item>
                  </div>
                    ...
                    <Form.Item label="Фавикон">
                      ...
                      >
                        <Button icon={<UploadOutlined />}>
                          {settings.faviconUrl ? 'Изменить фавикон' : 'Загрузить фавикон'}
                        </Button>
                      </Upload>
                      {settings.faviconUrl && (
                        <div style={{ marginTop: 16 }}>
                          <img
                            ...
                            alt="Favicon preview"
                            style={{ width: 32, height: 32, objectFit: 'contain', border: '1px solid #d9d9d9', borderRadius: 4 }}
                          />
                          <Button
                            danger
                            type="text"
                ...
                children: (
                  <div style={{ padding: 16 }}>
                    ...
                    <List
                      ...
                      renderItem={item => (
                        <List.Item
                          actions={[<Button key="del" type="text" danger icon={<DeleteOutlined />} onClick={() => handleDeleteFont(item.id)} />]}
                        >
                          <List.Item.Meta
                            title={item.name}
            ...
            >
              ...
              <div style={{ 
                ...
                <div style={{ 
                  ...
                  <div style={{ width: '100%', height: 200, flex: '1 0 auto', background: '#ddd', borderRadius: 'max(0px, calc(var(--card-radius) - var(--card-padding)))' }}></div>
                  <div style={{ 
                    fontFamily: 'var(--font-family-body)',
              ...
              <div style={{ marginBottom: 'calc(var(--base-gap) * 3)' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 'calc(var(--base-gap) * 0.5)' }}>
                  <a href="#" style={{ 
                    fontFamily: 'var(--font-family-body)',
                    fontSize: 'calc(var(--scale) * var(--body-size))',
